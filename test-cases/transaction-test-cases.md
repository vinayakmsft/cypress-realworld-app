...
it("should reject payment with insufficient funds", () => {
  cy.database("filter", "users").then((users: User[]) => {
    ...
    cy.getBySel("transaction-create-amount-input-error").should("contain", "insufficient funds");
  });
});
```
...
describe("Payment Amount Validation", () => {
  ...
  it("should reject negative amounts", () => {
    ...
    cy.getBySel("transaction-create-submit-payment").click();
    cy.getBySel("transaction-create-amount-input-error").should(
      "contain",
...
it("should decline payment request successfully", () => {
  cy.database("filter", "users").then((users: User[]) => {
    ...
    cy.getBySel("notification-list-item").should("not.contain", "Test request");
  });
});
```
...
it("should add comment to transaction", () => {
  cy.database("filter", "users").then((users: User[]) => {
    ...
    cy.getBySel("transaction-comments-list").should("contain", commentText);
...
it("should like and unlike transaction", () => {
  cy.database("filter", "users").then((users: User[]) => {
    ...
    cy.get("@initialCount").then((initialCount) => {
      ...
      cy.getBySel("transaction-like-count").should("contain", expectedCount.toString());
    });
    ...
    cy.get("@initialCount").then((initialCount) => {
      cy.getBySel("transaction-like-count").should("contain", initialCount as string);
    });
  });
});
```
